...
export default function MenMainboardPage() {
  ...
  useEffect(() => {
    const loadModels = async () => {
      ...
      try {
        const url = search 
          ? `/api/models?search=${encodeURIComponent(search)}&gender=male&category=mainboard` 
          : '/api/models?gender=male&category=mainboard';
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      const url = query.trim() 
        ? `/api/models?search=${encodeURIComponent(query)}&gender=male&category=mainboard` 
        : '/api/models?gender=male&category=mainboard';
      const response = await fetch(url);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-base sm:text-lg text-white max-w-3xl mx-auto">
              Discover our mainboard male talent
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">No male mainboard models found</p>
            </div>
          ) : (
