...
export default function MenNewFacesPage() {
  ...
  useEffect(() => {
    const loadModels = async () => {
      ...
      try {
        const url = search 
          ? `/api/models?search=${encodeURIComponent(search)}&gender=male&category=new-faces` 
          : '/api/models?gender=male&category=new-faces';
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      const url = query.trim() 
        ? `/api/models?search=${encodeURIComponent(query)}&gender=male&category=new-faces` 
        : '/api/models?gender=male&category=new-faces';
      const response = await fetch(url);
      ...
      <div className="pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">No male new faces found</p>
            </div>
          ) : (
